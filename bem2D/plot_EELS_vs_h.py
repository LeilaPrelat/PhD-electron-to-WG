...
title = r'$W$ = %i nm, $h$ = %i nm, $z_{\text{e}} = %i$ nm, $x_{\text{e}} = %i$ nm, $E_{\rm e}$ = %i keV' %(w,h,ze,xe,Ee_electron_keV)
Nint = 50
...
for j in range(len(listN)):
    ...
    plt.plot(listeV_correct  ,listEELS_correct   ,'.-',lw =lw, color = list_colors[j], label = r'$N$ = %i' %(N))
...
plt.tight_layout()
os.chdir(path_data)
plt.savefig( 'EELS_%ihnm_Ee%ikeV.png' %(h,Ee_electron_keV),bbox_inches='tight',pad_inches = 0.01, format='png', dpi=dpi)
plt.show()
