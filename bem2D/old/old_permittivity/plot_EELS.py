...
title = r'$W$ = %i nm, $h$ = %i nm, $E_{\rm e}$ = %i keV, $\epsilon$ = 12.25 + i0.2' %(W,h,Ee_electron_keV)
Nint = 50
...
plt.figure(figsize=tamfig)
plt.title(title,fontsize=tamtitle)
plt.xlabel(r'Photon energy $\hbar\omega$ (eV)',fontsize=tamletra,labelpad =labelpadx)
plt.ylabel(r'EELS with L = %i nm'%(L),fontsize=tamletra,labelpad =labelpady)
for mode in list_mode1:
    kk = 0
    for N in listN:
        ...
        #name3 = 'EELS_comsol2_spectrum_N%i_W%inm_h%inm_L%inm_Ee%ikeV_mode%i.dat' %(N,W,h,L,Ee_electron_keV,mode) ## virtual geometry
        ...
        if mode == list_mode1[0]:
            plt.plot(listeV, listEELS,'.-',lw=lw,color = list_colors[kk],label = r'$N = %i$' %(N))
            plt.plot(listeV[peaks], listEELS[peaks], "x",color = list_colors[kk])
        else:
            plt.plot(listeV, listEELS,'.-',lw=lw,color = list_colors[kk])
            plt.plot(listeV[peaks], listEELS[peaks], "x",color = list_colors[kk])
        ...
        print("L = %i nm, N = %i, W = %i nm, h = %i nm" %(L,N,W,h))
...
plt.savefig( 'EELS_convergency_L%inm.pdf'%(L),bbox_inches='tight',pad_inches = 0.01, format='pdf', dpi=dpi)
plt.show()
...
plt.title(title,fontsize=tamtitle)
plt.xlabel(r'Photon energy $\hbar\omega$ (eV)',fontsize=tamletra,labelpad =labelpadx)
plt.ylabel(r'EELS with N = %i nm'%(N0),fontsize=tamletra,labelpad =labelpady)
for mode in list_mode2:
    kk = 0
    for L in listL:
        ...
        if mode == list_mode2[0]:
            plt.plot(listeV, listEELS,'.-',lw=lw,color = list_colors[kk],label = r'$L = %i$' %(L))
            plt.plot(listeV[peaks], listEELS[peaks], "x",color = list_colors[kk])
        else:
            plt.plot(listeV, listEELS,'.-',lw=lw,color = list_colors[kk])
            plt.plot(listeV[peaks], listEELS[peaks], "x",color = list_colors[kk])
        ...
        try: 
           x_left, x_right, x0_fit, function_lorentzian, fit_success = find_width_of_peak(listeV_correct,listEELS_correct,-1,title,0)
           print("L = %i nm, xleft = %.3f eV, xright = %.3f eV, N = %i, W = %i nm, h = %i nm" %(L,x_left,x_right,N,W,h))
        except TypeError: 
            continue
        kk = kk + 1
        
        name_zoom = 'EELS_comsol2_spectrum_N%i_W%inm_h%inm_L%inm_Ee%ikeV_zoom.dat' %(N0,W,h,L,Ee_electron_keV) ## virtual geometry
        try:
            tabla1 = np.loadtxt(name_zoom, delimiter=' ',dtype=None)
            tabla1_2 = np.transpose(tabla1)
...
plt.savefig('EELS_convergency_virtual_geometry_N%i.pdf'%(N),bbox_inches='tight',pad_inches = 0.01, format='pdf', dpi=dpi)
plt.show()
