...
for j in range(len(list_b_nm)):
    ...
    # plt.plot(list_energy_eV[peaks], listy[peaks], "x")
    
    ## interpolation
    x_int = np.linspace(np.min(list_energy_eV),np.max(list_energy_eV),int(len(list_energy_eV))*100)
    y_int = np.interp(x_int, list_energy_eV, listy) 
    
    y_max = np.max( listy[peaks])
    ind_max = np.argmax(listy[peaks])
    x_max = list_energy_eV[peaks][ind_max]
...
print('2b-Plot the EELS integrated over k_par, over the trajectory, and over energy close to the highest peak for different b')
...
list_b_nm_longer = np.linspace(1,50,11)
delta_hbw = 0.05
list_EELS_re_tot = []
list_EELS_im_tot = []
for b_nm in list_b_nm_longer:
    b = b_nm*1e-3
    list_EELS_re = []
    list_EELS_im = []
    k = 0
...
plt.xlabel(labelx,fontsize=tamletra,labelpad =labelpadx)
plt.ylabel(labely,fontsize=tamletra,labelpad =labelpady)
for j in range(len(list_b_nm_longer)):
    plt.plot(list_b_nm_longer, np.array(list_EELS_re_tot[j]) ,'-',color = color1[j],lw = 1.5,label = r'$b = %i$ nm' %(list_b_nm[j]) )
