...
// Here, I approximated the function potential to a linear function 
...
double slope, offset; //from run_potential_norm_V0.sh: V(z)/V0 = slope_fit*z/W + offset_fit      
...
// load the fit of the potential (slope and offset) from running run_potential_approx.sh
void load_fit_of_potential_from_file(double wp_norm_w, double d_norm_w, double h_norm_w, double xe_norm_w, double zmin_norm_w){
    FILE *inputFile_potential;
    char filename_potential[256];     // Define file names for important the data (strings of lenght up to 1000)
    
    double b_norm_w=zmin_norm_w-h_norm_w;
    
    sprintf(filename_potential, "potential_data/linear_fit_wp%.2f_h%.2f_d%.2f_xe%.1f_bmin%.2f.txt",wp_norm_w,h_norm_w,d_norm_w,xe_norm_w,b_norm_w);    
...
//printf("opening %s\n", filename_potential);
    ...
    while (fgets(line, sizeof(line), inputFile_potential)) {
        // Skip comment lines (starting with # or empty lines)
        if (line[0] == '#' || line[0] == '\n') {
            continue;
        }

        // Try to parse two doubles
        if (sscanf(line, "%lf %lf", &slope, &offset) == 2) {
          //  printf("slope = %.15f, offset = %.15f\n", slope, offset);
        }
    }
...
double P_integrated_approx(double zmin_norm_w, double energy_eV, double w, double wp_norm_w, double h_norm_w, double d_norm_w, double x0_norm_w, int Eelectron_keV, int N, double theta_mrad, double V0, int save_P_vs_z) {
    
    double theta_rad=theta_mrad*1e-3;
    // Constants
    double me_c2_eV = 510998.95069; // me*c^2 in eV
   ...
   double integral = 0.0;
    ...
    // Load fit of the potential: slope, offset 
    load_fit_of_potential_from_file(wp_norm_w,d_norm_w,h_norm_w,x0_norm_w,zmin_norm_w);
    ...
    if (fabs(V0) < 1e-12) {
    printf("Error: V0 is too small (%.6e)\n", V0);
    return 1;
}

 
    
    double zmin_norm_w_from_fit = (0.5*me_c2_eV*gamma_e*aux*aux/V0 + offset)/(-1.0*slope); // this should be the same as zmin_norm_w because the fit works in this range (should include this value of zmin) 
    
 if (ABS(zmin_norm_w_from_fit - zmin_norm_w) > 0.05) {   // tolerance for bad fitting
        printf("Values zmin/w from fit = %.5f and zmin/w real = %.5f differ (stop calculation)\n", zmin_norm_w_from_fit, zmin_norm_w);
        printf("me_c2_eV = %.6e, gamma_e = %.6e, aux = %.6e, V0 = %.6e\n", me_c2_eV, gamma_e, aux, V0);
        return 1;
    }
...
// integration over z
    for (int i = 0; i < Nint; i++) {
        ...
        double Vz_norm_V0_approx =  slope*zval + offset; // approximation of phi to the a linear function . a1 and b1 are from the fit --> we load a1 and b1 instead of loading the potential
      ...
      //  printf("z/w=%.8f, EELS(z/w)=%.8f", zval, EELSz);
        ...
        double arg = ABS(aux * aux + 2.0 * Vz_norm_V0_approx * V0 / (me_c2_eV * gamma_e));   // second term is with + because Vz_norm_V0 < 0 
...
//printf("eV=%.4feV, z/w=%.7f, zmax/w = %.8f , zmin/w = %.8f\n", energy_eV, zval, zmax_norm_w, zmin_norm_w);
   ...
   sprintf(filename_Pintegrand, "EELS_integrated/P_integrand_vs_z_approx%s.dat", all_info_label);
