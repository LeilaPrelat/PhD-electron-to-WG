#include "complex.h"
#include "jga.h"
#include "EELS_integrated_over_z.h"
...
double P_integrated(double zmin, double energy_eV, int w, double wp_norm_w, double h_norm_w, double x0_norm_w, int Eelectron_keV, int N, double theta_mrad, double V0);
...
void load_energy_from_EELS_files(int w, double h, double x0_norm_w, int Eelectron_keV, int N){
 ...
 //   }
    ...
    sprintf(filename_energy, "datfiles_EELS/new_files/energy_list_N%i_W%.0dnm_h%.0fnm_L%inm_Ee%ikeV_xe%.2f.txt",N,w,h,L,Eelectron_keV,x0_norm_w);    
...
int main() {
    ...
    load_energy_from_EELS_files(w, h, x0_norm_w, Eelectron_keV, N);
    ...
    for (int j = 0; j < Nsolutions; j++) {
        ...
        for (int k = 0; k < num_energy_points; k++) {
            ...
            double result = P_integrated(z_min_val_norm_w, energy, w, wp_norm_w, h_norm_w, d_norm_w, x0_norm_w, Eelectron_keV, N, theta_mrad, V0, save_integrand);
